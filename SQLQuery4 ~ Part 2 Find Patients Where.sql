...
FROM 
	patient p 
	INNER JOIN observation o ON p.["registration_guid"] = o.["registration_guid"]
	INNER JOIN clinical_codes c ON (o.["emis_code_id"] = c.["code_id"] AND o.["snomed_concept_id"] = c.["snomed_concept_id"])
	LEFT OUTER JOIN medication m ON p.["registration_guid"] = m.["registration_guid"]
WHERE 
